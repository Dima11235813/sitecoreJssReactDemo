...
class DyanmicDisplayOfBlogArticles extends React.Component {
  ...
  render() {
    ...
    } else if (this.pageEditing && this.state.cardDataToRender && this.state.authorContent) {
      let blogCount = 0
      let styleForLabel = {
        width: "100%"
      }
        ...
        <div className="blog-list-container">
          {this.state.cardDataToRender.item.children.map(item => {
            if (item.name !== "Page Components") {
              ...
              //name
              const nameJss = name.jss ? name.jss : noAttributeIndicator
              const nameJssExists = name === noAttributeIndicator
              //tileDescription
              const tileTitleJss = tileTitle.jss ? tileTitle.jss : noAttributeIndicator
              const tileTitleJssExists = tileTitle === noAttributeIndicator
              //description
              ...
              //image
              const tileImageJss = tileImage.jss ? tileImage.jss : noAttributeIndicator
              const tileImageJssExists = tileImageJss === noAttributeIndicator
              return (
                <div className="blog-item" key={`${blogCount}blog`}>
                  <div style={styleForLabel}>Name</div>
                  {nameJssExists ? <Text field={nameJss}></Text> : noAttributeIndicator}
                  ...
                  <div style={styleForLabel}>Image</div>
                  {tileImageJssExists ? <Text field={tileImageJss}></Text> : noAttributeIndicator}
                </div>
              )
            }
          })}
